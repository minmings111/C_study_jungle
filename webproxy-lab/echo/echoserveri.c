 ...
 *
 * 중요한 점:
 * - echoserveri.c가 "서버 프로그램" 역할을 한다.
 * - echo.c는 서버가 필요할 때 호출해서 쓰는 "처리 함수" 역할을 한다.
 * - 즉, echo.c가 서버를 실행시키는 것이 아니라,
 *   서버가 실행된 뒤에 echoserveri.c가 echo()를 호출한다.
 */
 ...
 *
 * 이 함수의 역할을 한 문장으로 정리하면:
 * - "진짜 서버 역할"을 하는 함수이다.
 *
 * 즉, 이 함수가 하는 일은:
 * - 서버 포트를 열고
 * - 클라이언트 접속을 기다리고
 * - 접속이 오면 echo()를 호출하는 것이다.
 *
 * 매개변수 설명:
 * - argc:
 ...
 *
 * 여기서 중요한 점:
 * - argc 안에는 "8080"이 들어가지 않는다.
 * - argc에는 숫자 2가 들어간다.
 * - 실제 포트 번호 문자열 "8080"은 argv[1]에 들어간다.
 *
 * 아주 짧게 정리하면:
 * - argc = 몇 개 입력했는가
 * - argv = 무엇을 입력했는가
...
int main(int argc, char **argv) {
    int listenfd; // "서버가 손님을 기다릴 때 쓰는 소켓 번호", 운영체제가 정수로 관리하므로 int
