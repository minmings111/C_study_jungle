...
즉, 이 파일은 서버를 실행하는 파일이 아니라,
서버가 접속을 받은 뒤 호출하는 "메아리 처리 함수"가 들어 있는 파일입니다.
...
- `echoserveri.c`: 서버를 열고, 기다리고, `echo()`를 호출하는 파일
- `echo.c`: 서버가 넘겨준 연결로 실제 echo 동작을 수행하는 파일
- `echoclient.c`: 서버에 접속해서 문자열을 보내는 파일
...
- `argc` 안에는 `"8080"` 같은 문자열이 들어가지 않습니다.
- `argc`에는 `2`, `3` 같은 개수만 들어갑니다.
- `"8080"` 같은 실제 문자열은 `argv[1]` 또는 `argv[2]` 같은 위치에 들어갑니다.
